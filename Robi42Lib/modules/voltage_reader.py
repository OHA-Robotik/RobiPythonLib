...
def to_voltage(X : float, ref_voltage : float) -> float:
...
def spannungsteiler(u_in : float, r1 : float, r2 : float) -> float:
    """Berechne die Spannung am ADC, wenn du die Spannung am Eingang kennst."""
    return u_in * (1 / (1 + r2/r1))


def spannungsteiler_reverse(u_out, r1, r2) -> float:
...
class VoltageReader(base_module.BaseModule):
    ...
    def get_battery_voltage(self) -> float:
        ...
        return self.pin_battery.read_raw() / self.magic_bat

    def get_5v_voltage(self) -> float:
        """Returns the 5V voltage in V"""
        return self.pin_50v.read_raw() / self.magic_50v

    def get_33v_voltage(self) -> float:
        """Returns the 3.3V voltage in V"""
        return self.pin_33v.read_raw() / self.magic_33v
