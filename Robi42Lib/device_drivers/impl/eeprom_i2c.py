...
# Stole from https://github.com/peterhinch/micropython_eeprom/tree/master/eeprom/i2c
...
import machine
from ...hardware_manager import platform_description
...
class EEPROM_24LC256_I2C_driver(base_driver.I2C_BaseDriver, EEPROM):
    # All address pins low                   # all address pins high
    SUPPORTED_ADDRESSES = {0x50}  # , 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57

    def __init__(self, i2c_interface: machine.I2C, i2c_addr: int):
        base_driver.I2C_BaseDriver.__init__(self, i2c_interface, i2c_addr)
        driver_prefs = platform_description.PlatformLoader.get_instance().get_preference_for_driver('24lc256_i2c')
        EEPROM.__init__(self, self.i2c_interface, T24C256,
                        driver_prefs['verbose'], driver_prefs['block_size'], self.i2c_addr, 1,
                        driver_prefs['page_size'])
