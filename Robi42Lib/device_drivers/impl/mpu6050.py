...
class MPU6050(base_driver.I2C_BaseDriver):
    ...
    def __init__(self, i2c: I2C, address: int) -> None:
        ...
        self.i2c_interface.writeto_mem(address, self.PWR_MGMT_1, b"\x00")
        utime.sleep_ms(100)
        self.i2c_interface.writeto_mem(address, self.SMPLRT_DIV, b"\x07")
        self.i2c_interface.writeto_mem(address, self.CONFIG, b"\x00")
        self.i2c_interface.writeto_mem(address, self.GYRO_CONFIG, b"\x00")
        self.i2c_interface.writeto_mem(address, self.ACCEL_CONFIG, b"\x00")

    def read_raw_data(self, addr):
        high = self.i2c_interface.readfrom_mem(self.i2c_addr, addr, 1)[0]
        low = self.i2c_interface.readfrom_mem(self.i2c_addr, addr + 1, 1)[0]
        value = high << 8 | low
        if value > 32768:
            value -= 65536
        return value
