...
class VL53L0X(base_driver.I2C_BaseDriver):
    SUPPORTED_ADDRESSES = {0x29}
    ...
    def _registers(self, register, values=None, struct="B"):
        if values is None:
            ...
            data = self.i2c_interface.readfrom_mem(self.i2c_addr, register, size)
            values = ustruct.unpack(struct, data)
            return values
        data = ustruct.pack(struct, *values)
        self.i2c_interface.writeto_mem(self.i2c_addr, register, data)
